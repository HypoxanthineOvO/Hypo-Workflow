...
- 2026-05-17T21:41:21+08:00：完成 v12.8.1 release 准备；版本源、README/README.en、CHANGELOG、中英文 release notes、OpenCode artifacts 和 docs registry 已同步到 12.8.1。发布前修复 PR lifecycle log schema 漏项（`pr` family 与 review/merge 状态），解除完整回归阻塞；log-evidence 5/5、focused docs/log 13/13、focused docs/readme/skill/sync/platform 42/42、完整 Node suite 516/516、完整 regression 68/68、docs/readme checks、OpenCode sync check-only 和 `git diff --check` 均通过。下一步执行 release commit、tag、GitHub/GitLab push 和 GitHub release publish。
- 2026-05-17T21:24:54+08:00：按用户授权接收并合入 GitHub PR #8；先提交 approve review，再以普通 merge commit 合入，GitHub merge commit 为 `930b5620c4d9d0f4ccd862f434a9497e664c285e`，本地 `main` 已 fast-forward。合入后本地 docs governance API 通过；PR worktree 的 sync check-only 仅剩 `.pipeline` compact/runtime 派生物 stale warning，未作为 PR payload 合入。
- 2026-05-17T21:18:42+08:00：复审 GitHub PR #8 新提交 `27c5389`；此前 request-changes 指出的 Cursor `/hw-setup` 未镜像 `references/config-spec.md` 问题已解决，生成 Skill 现在自包含并有 external/non-local fallback，新增 reference 可解析回归检查。隔离 worktree 中 PR 自带测试 13/13、扩展 smoke 29/29、完整 Node suite 516/516、`git diff --check main...github/pr/8` 均通过；当前建议可 approve，但仍需显式授权后才写远端 approval。
- 2026-05-17T21:08:04+08:00：按用户确认，在 GitHub PR #8 提交 `request changes` review；回复内容说明整体方向认可、本地测试通过，并要求修复 Cursor `/hw-setup` 引用未镜像 `references/config-spec.md` 的契约缺口后再复审。
- 2026-05-17T21:03:42+08:00：完成 GitHub PR #8（Baojch，`Optimize Cursor adapter sync`）本地只读 inspect/review 归档；PR mergeable 但远端无 reported checks，153 files changed，主要是 Cursor flat skills/commands 与 `.cursor/hypo-workflow/` 资源镜像。隔离 worktree 中 PR 自带测试 13/13、扩展 smoke 29/29、完整 Node suite 516/516 均通过；当前建议 request changes，原因是生成的 Cursor `/hw-setup` 指向未随 bundle 镜像的 `references/config-spec.md`。详见 `.pipeline/pr/PR-20260517-001/review-notes.md`。
