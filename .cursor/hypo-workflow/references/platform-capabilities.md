...
|---|---|---|---|---|---|---|
| User command surface | `/hw:*` skill invocation | `/hw:*` plugin slash commands backed by Skills | `/hw-*` native slash commands | repository instructions + `.cursor/commands/hw-*.md` | repository instructions | repository instructions |
| Instruction surface | `SKILL.md` + rules files | Claude plugin `commands/`, skills, hooks, and agents | OpenCode command/agent files plus plugin context | `.cursor/rules/hypo-workflow.mdc` + `.cursor/skills/hw-*.md` + compact `.cursor/hypo-workflow/` resources | `.github/copilot-instructions.md` | `.trae/rules/project_rules.md` |
...
| Rules/instructions | skill/rules files | plugin context | `AGENTS.md`, instructions, plugin context | generated rule + per-command Skill files | generated instruction | generated rule |
...
- Cursor, Copilot, and Trae adapters must not claim lifecycle hook enforcement unless a later implementation adds real host support; Cursor's flat Skill and command files are instruction/resource surfaces, not a runner.
...
| Cursor | `docs/platforms/cursor.md` | `.cursor/rules/hypo-workflow.mdc`, `.cursor/skills/hw-*.md`, `.cursor/commands/hw-*.md`, and `.cursor/hypo-workflow/` |
...
|---|---|---|
| Cursor | `.cursor/rules/hypo-workflow.mdc` + `.cursor/skills/hw-*.md` + `.cursor/commands/hw-*.md` | Project rule plus one flat Skill file and one slash command file per `/hw-*` entry. Command authority is embedded in flat Skills; only compact shared references/assets/scripts/adapters are mirrored under `.cursor/hypo-workflow/`. Model selection stays with the active Cursor UI/session. |
