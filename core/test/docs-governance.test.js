...
test("docs map defines ownership, generated references, and narrative policy", () => {
  ...
  const releaseNote = map.documents.find((doc) => doc.path === "docs/release/v14.0.0-alpha.6.md");
  const englishReleaseNote = map.documents.find((doc) => doc.path === "docs/en/release/v14.0.0-alpha.6.md");
...
test("v14.0.0-alpha.6 release coverage is Chinese-first and linked from entrypoints", async () => {
  const chineseRelease = await readFile("docs/release/v14.0.0-alpha.6.md", "utf8");
  const englishRelease = await readFile("docs/en/release/v14.0.0-alpha.6.md", "utf8");
  const readme = await readFile("README.md", "utf8");
  const englishReadme = await readFile("README.en.md", "utf8");

  assert.match(chineseRelease, /^# v14\.0\.0-alpha\.6 发布说明/m);
  assert.match(chineseRelease, /\[English\]\(\.\.\/en\/release\/v14\.0\.0-alpha\.6\.md\)/);
  assert.match(chineseRelease, /## Experiment 记录[\s\S]*## Hook 行为[\s\S]*## 验证/);
  assert.match(englishRelease, /^# v14\.0\.0-alpha\.6 Release Notes/m);
  assert.match(englishRelease, /\[中文\]\(\.\.\/\.\.\/release\/v14\.0\.0-alpha\.6\.md\)/);
  assert.match(englishRelease, /## Experiment Records[\s\S]*## Hook Behavior[\s\S]*## Verification/);
  assert.match(readme, /docs\/release\/v14\.0\.0-alpha\.6\.md/);
  assert.match(englishReadme, /docs\/en\/release\/v14\.0\.0-alpha\.6\.md/);
});
