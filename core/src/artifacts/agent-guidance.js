export const ASK_QUESTIONS_GUIDANCE = `## Ask Questions Discipline
...
export function renderDeepSeekToolCallingRules(model, host = "the host agent") {
  if (!isDeepSeekModel(model)) return "";
  return `## DeepSeek Tool Calling Rules

When using DeepSeek through ${host}, follow these rules strictly. They override any conflicting habits from chat training.
...
export function isDeepSeekModel(model) {
  return /(^|[/_-])deepseek([/_-]|$)/i.test(String(model || ""));
}
