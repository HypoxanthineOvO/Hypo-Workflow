...
function normalizeRejectionBlockedRequest(value = {}) {
  if (!value || typeof value !== "object") {
    return { status: "none", proposed_by_role: null, approved_by_role: null };
  }
  return {
...
function normalizeRejectionAudit(value = {}) {
  if (!value || typeof value !== "object") {
    return { reviewer_role: "audit", verdict: "unknown", findings: [] };
  }
  return {
    ...value,
...
function collectReworkRoles(requiredRework = []) {
  ...
  for (const item of requiredRework) {
    const ownerRoles = Array.isArray(item?.owner_roles) ? item.owner_roles : [];
