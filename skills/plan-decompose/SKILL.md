...
仅在 Decompose 阶段使用此 skill。旧文档中的 `P2` 是 Decompose 的兼容别名。

对于 `/hw:plan --batch`，此阶段根据 `batch.decompose_mode` 拆解 Feature Queue 条目。

## 前置条件

- Discover 已充分澄清项目，足以定义 Milestone
...
Decompose 不得以 goal-only checkpoint 进入 `proposed`。如果计划只列出目标、验收标准、功能队列、待办项或测试标题，而没有每个实现 Milestone 的技术方案、技术路线、调研状态、风险替代、验证路径和审计焦点，则 Decompose 必须保持 `in_progress` 或 `revision`。
...
每个 `research_required` 项必须在 Decompose 确认/Generate 之前处于以下状态之一：

- `resolved`：有本地代码、官方文档、用户说明或可运行探针作为证据
- `blocking_question`：已转成用户可回答的问题；Decompose 可展示为待答复检查点，但不得被确认或进入 Generate，直到用户回答或明确接受延后
- `deferred_by_user`：用户明确接受延后，并说明延后到哪个 Milestone 或风险门控

如果用户挑战技术路线、指出调研不足或要求比较替代方案，立即把 Decompose 状态视为 `revision` 或 `in_progress`，记录被挑战的字段和原因，执行目标调研，然后重新展示修订后的 Decompose 检查点。不要在挑战未处理时继续 Generate。

## 交互行为

- 在交互模式下，显示提议的 Milestone 拆分，并在依赖或范围边界仍不明确时提出后续问题
- Decompose 产出拆分后，在 Generate 之前的检查点停下
- 检查点必须显示：
  - Milestone 编号和名称
  - 目标
  - 实现范围
  ...
  - 未解决的假设
- 在进入 Generate 前先解释为什么需要确认、确认/要求修改分别会改变什么，然后使用 Question Tool / Ask 等待用户明确确认
- 不要从 Decompose 直接生成 `.pipeline/` 文件、prompt 文件或架构文件
...
交互式 Decompose 完成不等于允许写入文件。唯一有效的下一步是展示提议的拆解和技术路线并要求用户确认。只有在用户明确批准 Milestone 拆分、技术路线，并且硬性调研项已解决、已回答或已明确延后后，才能开始 Generate。

Decompose 可标记为 `proposed` 的最低条件：
...
- 没有 unresolved 的硬性 `research_required` 项；如果存在 `blocking_question`，检查点必须显示为等待用户答复，不能进入 Generate
...
3. 如果模式为 `upfront`，在 Generate 前将每个 Feature 拆解为初始 Milestone。
