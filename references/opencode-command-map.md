...
本表定义 canonical `/hw:*` 命令到 OpenCode native slash command 的映射。当前 OpenCode command surface 使用 canonical colon 形式；旧版 dash-style 文件名仅作为 adapter 清理兼容处理。命令名、agent 名、Skill 路径和路由字段必须保持英文 literal，不得翻译；中文说明只解释用途和边界。

OpenCode uses canonical colon-form native slash commands. Each command remains traceable to a canonical Hypo-Workflow command.
