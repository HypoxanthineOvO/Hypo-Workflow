...
- The final response must display the report's core content in conversation: conclusion/change summary, technical approach, modified files/modules, validation results, expected result, and risks/follow-up. A bare sentence such as "written to `.pipeline/.../report.md`" is insufficient even when the report exists.
- The final response must explain the substance of the report before or alongside artifact paths. It must not only list `.pipeline/...` paths, worker closures, YAML validity, and test counts. For each important report/review artifact, summarize what it contains, the main conclusion, the user-facing interpretation, and what the user should understand or do next.
- Learning gates, quizzes, research reports, requirement briefs, and design reports require domain explanation in the chat itself. The response should teach the key concepts, why they matter, how the report or quiz is structured, and the intended checkpoint outcome. Artifact paths are supporting references after that explanation, not a substitute for it.
