...
- Parallel tasks use separate Workstreams bound to a VSPi Session and `{ workspace_id, delivery_ref, plan_hash, revision }`.
...
VSPi loads the version-matched Core bundle and uses `createDeliveryStore`, `createWorkstreamStore`, `compileVspiIntegrationContract`, and `parseVspiIntegrationContract` with the project root, an explicit zero-argument Clock, and a unique mutation ID.
